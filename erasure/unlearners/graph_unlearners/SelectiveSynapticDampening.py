...
class SelectiveSynapticDampening(GraphUnlearner):
    def init(self):
        """
    ...
    def __unlearn__(self):
        ...
        self.train_set = self.dataset.partitions[self.ref_data_train]
        ...
        ssd = ParameterPerturber(self.predictor.model, self.predictor.optimizer, self.device, self.parameters, unlearner=self)
        self.predictor.model.eval()

        sample_importances = ssd.calc_importance(self.forget_set)

        original_importances = ssd.calc_importance(self.train_set)
...
class ParameterPerturber:
    ...
    def calc_importance(self, dataset) -> Dict[str, torch.Tensor]:
        """
        ...
        out = self.model(self.unlearner.x,self.unlearner.edge_index)[dataset]
        y = self.unlearner.labels[dataset]
