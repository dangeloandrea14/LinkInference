...
FLAT = True  # all configs go directly in BASE_DIR, no per-dataset subdirs
...
def main():
    ...
    for dataset_name, ds_cfg in DATASETS.items():
        for arch_name, arch_class in ARCHITECTURES.items():
            ...
            fname = os.path.join(BASE_DIR, f"{dataset_name}_{arch_name}_hard.jsonc")
            with open(fname, "w") as f:
                f.write(cfg)
            print(f"  created {fname}")
            created += 1

            # Easy config for GCN and GIN
            if arch_name in ("GCN", "GIN"):
                cfg = make_config(dataset_name, ds_cfg, arch_name, arch_class, "easy")
                fname = os.path.join(BASE_DIR, f"{dataset_name}_{arch_name}_easy.jsonc")
                with open(fname, "w") as f:
                    f.write(cfg)
                print(f"  created {fname}")
                created += 1
